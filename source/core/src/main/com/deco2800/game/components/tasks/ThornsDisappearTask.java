 ...
 */
public class ThornsDisappearTask extends DefaultTask implements PriorityTask {
    private final Entity target;
    private final int priority;
    private final float viewDistance;
    private final PhysicsEngine physics;
    private final DebugRenderer debugRenderer;
    private static final Logger logger = LoggerFactory.getLogger(ThornsDisappearTask.class);
     ...
     */
    public ThornsDisappearTask(Entity target, int priority, float viewDistance) {
        this.target = target;
    ...
    public int getPriority() {
        ...
        if (targetY > 5) {
            if (dst < (viewDistance + 1.5)) {
                logger.info("Player is too close to the thorns obstacles.");
                return priority;
            }
        } else {
            if (dst < viewDistance) {
